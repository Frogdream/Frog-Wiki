---
icon: martini-glass-empty
---

# Напитки

## Варочные аппараты

Напитки делятся на две подгруппы:  Те, которые готовятся на дистилляторе, и те, которые готовятся на варочном аппарате.

<figure><img src="../.gitbook/assets/2026-03-18_14.55.01.png" alt=""><figcaption></figcaption></figure>

Слева - дистиллятор. Справа - варочный аппарат

## Дистиллятор

На дистилляторе готовятся преимущественно алкогольные напитки.\
Для готовки каждого напитка закиньте все ингредиенты в котёл и ожидайте пока напиток выпадет.&#x20;

{% hint style="warning" %}
Обратите внимание, котел не должен быть наполнен водой!
{% endhint %}

{% hint style="info" %}
Если не подобрать напиток за определенное время, то он испортится. Испорченные напитки дают немного другие эффекты - как отрицательные, так и положительные.
{% endhint %}

### Бальзам

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (14).png" alt=""><figcaption></figcaption></figure>

Осколок призмарина, коричневый гриб, блок мха, бутылочка.
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (10).png" alt=""><figcaption></figcaption></figure>

Скорость I\
Удача I\
Отравление I\
Тошнота V
{% endcolumn %}
{% endcolumns %}

### Бренди

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (15).png" alt=""><figcaption></figcaption></figure>

Ламинария, светящаяся ягода, кроличья лапка, бутылочка
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (13).png" alt=""><figcaption></figcaption></figure>

Ночное видение I\
Регенерация I\
Медленность II\
Отравление I\
Неудача V\
Тошнота III
{% endcolumn %}
{% endcolumns %}

### Ликёр

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (17).png" alt=""><figcaption></figcaption></figure>

Светящаяся ягода, светокаменная пыль, сахар, кроличья лапка, бутылочка
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (19).png" alt=""><figcaption></figcaption></figure>

Сила I\
Скорость III\
Отравление III\
Утомление II\
Неудача II\
Тошнота III
{% endcolumn %}
{% endcolumns %}

### Водка

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (22).png" alt=""><figcaption></figcaption></figure>

Кроличья лапка, пшеница, костная мука, семена пшеницы, бутылочка
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (23).png" alt=""><figcaption></figcaption></figure>

Иссушение I \
Медлительность II\
Удача I\
Тошнота V\
Утомление I
{% endcolumn %}
{% endcolumns %}

### Ром

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (25).png" alt=""><figcaption></figcaption></figure>

Сахар, свёкла, бамбук, папоротник, кувшинка, бутылочка
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (26).png" alt=""><figcaption></figcaption></figure>

Сила II\
Медлительность III\
Удача I\
Тошнота IV
{% endcolumn %}
{% endcolumns %}

#### Шампанское

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (28).png" alt=""><figcaption></figcaption></figure>

Светящаяся ягода, яблоко, сладкая ягода, бутылочка.
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (27).png" alt=""><figcaption></figcaption></figure>

Огнестойкость I\
Поглощение I\
Голод II
{% endcolumn %}
{% endcolumns %}



## Варочный аппарат

Варочный аппарат предназначен преимущественно для неалкогольных или слабо алкогольных напитков.&#x20;

### Эль-Кубелиус

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (29).png" alt=""><figcaption></figcaption></figure>

Рудное золото, возвратный компас, незеритовый лом, черепаший щиток.
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (30).png" alt=""><figcaption></figcaption></figure>

**Неизвестно**
{% endcolumn %}
{% endcolumns %}

### Какао

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (32).png" alt=""><figcaption></figcaption></figure>

Светокаменная пыль, какао-бобы, бутылочка.
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (31).png" alt=""><figcaption></figcaption></figure>

Снимает все эффекты с игрока, а также дает Регенерацию I
{% endcolumn %}
{% endcolumns %}

### Кофе&#x20;

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (33).png" alt=""><figcaption></figcaption></figure>

Какао-бобы, сахар.
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (34).png" alt=""><figcaption></figcaption></figure>

Скорость I\
Прилив здоровья II
{% endcolumn %}
{% endcolumns %}

### Лимонады

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (35).png" alt=""><figcaption></figcaption></figure>

Золотое яблоко, бутылочка.
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (37).png" alt=""><figcaption></figcaption></figure>

Слепота I\
Прыгучесть I
{% endcolumn %}
{% endcolumns %}

### Пиво

{% columns %}
{% column %}
<figure><img src="../.gitbook/assets/изображение (38).png" alt=""><figcaption></figcaption></figure>

Хлеб, картофель, сушёная ламинария.
{% endcolumn %}

{% column %}
<figure><img src="../.gitbook/assets/изображение (39).png" alt=""><figcaption></figcaption></figure>

Насыщение I\
Утомление I\
Медлительность I\
Тошнота III
{% endcolumn %}
{% endcolumns %}
